...
@webvizstore
def get_table_df(
        csv_table: tuple) -> pd.DataFrame:
    return pd.read_csv(csv_table)

@CACHE.memoize(timeout=CACHE.TIMEOUT)
@webvizstore
def get_multiple_table_df(
        tables: tuple) -> pd.DataFrame:
    table_list = []
...
def get_summary_df(
    ...
    for case_name, case_path in case_paths:
        smry = EclSum(case_path.replace(".DATA", ".SMSPEC"), include_restart=False,
                      lazy_load=False).pandas_frame(None, column_keys)
...
@webvizstore
def load_parameters(
        ensemble_paths: tuple) -> pd.DataFrame:
...
def eclsum_keyword2title(keyword):
    ...
    if _title is None:
        ...
        if keyword.find(':') > 0:
            return ''.join(title_list) + " ({})".format(keyword[keyword.find(':') + 1:])
        else:
...
def create_trace_dict(x, y, curve_name, legend_name, opacity, showlegend, color, meta, xaxis, yaxis):
    return {
        'x': x,
        'y': y,
        'legendgroup': legend_name,
