...
"""
        ...
        return html.Div(
            ...
            children=[
                wcc.Graph(id=self.ids("graph"),
                          figure=self.plot_histogram_dict()),
            ],
        )
    ...
    def create_hist_layout(self, parameters):
        ...
        box_height = box_plot_height / \
            (box_plot_height + hist_plot_height)*chart_height
        hist_height = hist_plot_height / \
            (box_plot_height + hist_plot_height)*chart_height
        ...
        for idx_param, param in enumerate(self.parameter_columns):
            ...
            for idx_ens, ens in enumerate(ens_list):
                if self.best_realization:
                    data.append(self.create_best_trace(idx_param+1,
                                                       ens,
                                                       df[(self.parameters["ENSEMBLE"] == ens) & (
                                                           self.parameters["REAL"] == self.best_realization[idx_ens])]
                                                       )
                                )
                data.extend(self.create_hist_trace(idx_param+1,
                                                   df[self.parameters["ENSEMBLE"]
                                                       == ens].values,
                                                   color_list[idx_ens],
                                                   ens,
                                                   0.3,
    ...
    def add_webvizstore(self):
        return [
            (read_csv, [{"csv_file": self.csvfile}]) if self.csvfile
